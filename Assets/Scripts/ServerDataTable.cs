...
public class ServerDataTable : MonoSingleton<ServerDataTable>
{
	...
	public void LoadLangTableFromLocalFile()
	{
		string langFileName = LocalDataManager.GetLangFileName(GlobalSetting.GetSystemLanguage());
		string fileContents = LocalDataManager.GetFileContents(langFileName);
		if (!string.IsNullOrEmpty(fileContents))
		{
			PacketDataLanguageData[] res = JsonConvert.DeserializeObject<PacketDataLanguageData[]>(fileContents);
			LoadLangTable(res);
			return;
		}
		langFileName = LocalDataManager.GetLangFileName(SystemLanguage.English);
		fileContents = LocalDataManager.GetFileContents(langFileName);
		if (!string.IsNullOrEmpty(fileContents))
		{
			PacketDataLanguageData[] res2 = JsonConvert.DeserializeObject<PacketDataLanguageData[]>(fileContents);
			LoadLangTable(res2);
		}
	}
